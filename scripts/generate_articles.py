#!/usr/bin/env python3
"""
The Flightline — Daily Article Generation Script
...
SYSTEM_PROMPT = """You are a staff writer for The Flightline, a hyperlocal news publication covering \the Pensacola, FL metro area — Perdido Key to Cantonment, Pensacola Beach, Gulf Breeze, and downtown \Pensacola. The publication focuses on government transparency, public records, local development, and \community accountability.

VOICE & STYLE:
- Plain American English, AP style, no serial commas
- Scene-first openings — drop the reader into a specific moment or place
...
- These are feature and analysis pieces — not breaking news. Frame as such.
...
# ── Anthropic API call (raw HTTP, no SDK dependency on CI) ───────────────────
...
def build_email(articles, approval_url, today_str):
    ...
    return f"<!DOCTYPE html><html><body style='font-family:Georgia;max-width:620px;margin:32px auto;padding:24px;color:#111827;'><div style='background:#1e2d4a;padding:24px;margin-bottom:24px;'><div style='font-family:Arial Black;font-size:30px;font-weight:900;color:#fff;'>THE FLIGHTLINE</div><div style='font-size:12px;color:#d4952b;font-family:Arial;margin-top:4px;'>Pensacola, FL ₷ Daily Article Review</div></div><p style='font-family:Arial;font-size:15px;'>Claude generated <strong>{len(articles)} articles</strong> for <strong>{today_str}</strong>.</p>{cards}<div style='text-align:center;margin:32px 0;'><a href='{approval_url}' style='background:#1e2d4a;color:#fff;font-family:Arial;font-size:14px;font-weight:700;text-transform:uppercase;text-decoration:none;padding:14px 40px;border-radius:5px;display:inline-block;'>✓ APPROVE &amp; PUBLISH</a><div style='font-family:Arial;font-size:11px;color:#9ca3af;margin-top:10px;'>Link expires in 48 hours.</div></div></body></html>"
...
def main():
    ...
    print(f"  → Topics: {[t['headline'] for t in topics]}")

    articles = []
    for i, topic in enumerate(topics[:ARTICLES_COUNT], 1):
        print(f"  → Writing article {i}/{ARTICLES_COUNT}: {topic['headline']}")
        try:
            articles.append(write_article(topic, article_date))
        except Exception as e:
            print(f"  ✗ Failed on article {i}: {e}", file=sys.stderr)
    ...
    if GITHUB_ENV:
        with open(GITHUB_ENV, "a") as f:
            f.write(f"ARTICLE_DATE=t��ICLE_DATE}\n")
